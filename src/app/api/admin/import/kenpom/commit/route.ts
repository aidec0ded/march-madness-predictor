/**
 * API Route: POST /api/admin/import/kenpom/commit
 *
 * "Confirm & Save to Database" endpoint for the KenPom multi-CSV import
 * pipeline. Receives validated + normalized team data from the preview step
 * (POST /api/admin/import/kenpom) and upserts it into Supabase.
 *
 * Request body:
 * ```json
 * {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    logger.info("KenPom commit: teams upserted", {
    ...
    // -----------------------------------------------------------------------
    // Step 4: Build team_season upsert records
    // -----------------------------------------------------------------------
    ...
    const teamSeasonRecords = typedTeams
      .map((t) => {
        ...
        // Destructure fields from the normalized data
        const {
          ratings,
          fourFactorsOffense,
          fourFactorsDefense,
          ...
          ratings?: { kenpom?: { adjOE: number; adjDE: number; adjEM: number } };
          fourFactorsOffense?: { efgPct: number; toPct: number; orbPct: number; ftRate: number };
          fourFactorsDefense?: { efgPct: number; toPct: number; orbPct: number; ftRate: number };
          shootingOffense?: { threePtPct: number; threePtRate: number; ftPct: number };
          shootingDefense?: { threePtPct: number; threePtRate: number; ftPct: number };
          adjTempo?: number;
        ...
        // Merge data_sources: existing + "kenpom" (deduplicated)
        ...
        const record: { team_id: string; season: number; [key: string]: unknown } = {
          ...
          experience: nanToNull(experience),
          minutes_continuity: nanToNull(minutesContinuity),
          avg_height: nanToNull(avgHeight),
          // Style
          two_foul_participation: nanToNull(twoFoulParticipation),
          // Merge data_sources
    ...
    if (skippedTeams.length > 0) {
      logger.warn("KenPom commit: skipped teams without ID mapping", {
    ...
    logger.info("KenPom commit: team_seasons upserted", {
    ...
    logger.info("KenPom commit: name mappings upserted", {
    ...
    return NextResponse.json({
      success: true,
      message: `KenPom data committed for season ${seasonNum}. ${seasonsUpserted} team seasons saved.`,
      data: {
        season: seasonNum,
        source: "kenpom",
